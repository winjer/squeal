...
class Group(AddHelperMixin, object):
    ...
    def process(self, ctx, form, args, errors):
        return defer.gatherResults([
            defer.maybeDeferred(item.process, ctx, form, args, errors)
...
class FormItems(object):
    ...
    def add(self, item):
        return self.__add(item, self.items.append)


    def addLast(self, item, ref=None):
        if ref is not None:
            return self.__refToItem(ref).addLast(item)
        return self.add(item)


    def addFirst(self, item, ref=None):
        if ref is not None:
            return self.__refToItem(ref).addFirst(item)
        return self.__add(item, lambda item: self.items.insert(0, item))


    def addBefore(self, item, ref):
        ref = self.__refToItem(ref)
        if ref.itemParent is not self.itemParent:
            return ref.itemParent.addBefore(item, ref)
        pos = self.items.index(ref)
        return self.__add(item, lambda item: self.items.insert(pos, item))


    def addAfter(self, item, ref):
        ref = self.__refToItem(ref)
        if ref.itemParent is not self.itemParent:
            return ref.itemParent.addAfter(item, ref)
        pos = self.items.index(ref)
        return self.__add(item, lambda item: self.items.insert(pos+1, item))


    def __refToItem(self, ref):
        if isinstance(ref, (str, unicode)):
            return self.getItemByName(ref)
        return ref


    def __add(self, item, adder):
